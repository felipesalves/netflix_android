...
class LoginInteractor @Inject constructor(
    ...
    suspend fun login(email: String?, password: String?): AppResultado<Nothing> {
        val (credential, ex) = validadeEmailAndPassword(email, password)
        return when (ex) {
            null -> repo.login(credential!!.first, credential.second)
            else -> AppResultado.Error(ex.localizedMessage, ex)
        }
    }

    suspend fun signup(email: String?, password: String?): AppResultado<Nothing> {
        val (credential, ex) = validadeEmailAndPassword(email, password)
        return when (ex) {
    ...
    suspend fun forgot(email: String?): AppResultado<Nothing> {
